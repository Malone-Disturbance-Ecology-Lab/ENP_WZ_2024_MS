...
# long term coastal
enphc <- df %>%
 dplyr::select(yearmon, ENPHC_mean_mon_WL)
enphc <-enphc[7:348,] 
...
ts1 <- df %>% dplyr::select(yearmon, TS1_mean_mon_WL)
...
ts7 <- df %>% dplyr::select(yearmon, TS7_mean_mon_WL)
...
# Salinity Analysis ####
enphc.sal <- df %>%
 dplyr::select(yearmon, ENPHC_mean_mon_Sal)
...
# Add TS1 and TS7
ts1.sal <- df %>% dplyr::select(yearmon, TS1_mean_mon_Sal)
...
ts7.sal <- df %>% dplyr::select(yearmon, TS7_mean_mon_Sal)
