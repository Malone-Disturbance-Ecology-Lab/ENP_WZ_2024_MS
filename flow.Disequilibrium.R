...
plot.ts1.dde <- ts1.gf.opt %>% group_by(TIMESTAMP) %>% summarise(NEE.gc = sum(NEE.gc) , NEE.Modeled.opt.gc = sum(NEE.Modeled.opt.gc)) %>% 
  ...
  ylab(expression(paste('NEE ( g C  m'^2, ' day'^-1, ')'))) + theme(text = element_text(size = 20),axis.text=element_text(size=20) , axis.text.x=element_text(angle = 60, vjust = 1, hjust = 1))
...
simulation.1 <- ggplot() + geom_sf(data = ENP.shp, fill='Transparent', col="black", linewidth=2) + geom_spatraster(data = veg$veg.p, na.rm = TRUE,show.legend = T) +  
  scale_fill_manual(na.value = "transparent", values= c("#43b284", "#000099", "#fab255"),
                    labels =c("Saline Scrub Mangrove","Freshwater Wetlands","Brakish Ecotone", ""))+
  labs( fill="" ) +
  geom_spatraster_contour(data = dist.fwm_raster, breaks =seq(1, 50, 1.5), na.rm = TRUE, col="lightblue4") +
  geom_sf(data= enp.shade, col='black', alpha=0.7, linewidth=2) + theme_bw()+theme(legend.position = "top") + theme(axis.text=element_text(size=20), text = element_text(size = 25))
