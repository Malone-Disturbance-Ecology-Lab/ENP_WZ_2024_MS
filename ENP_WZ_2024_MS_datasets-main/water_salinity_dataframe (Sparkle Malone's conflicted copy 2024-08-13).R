...
TSPH1_WL <- TSPH1_flux_met %>% mutate( TIMESTAMP= as.POSIXct(TIMESTAMP,
                                       ...
                                       Week = week(Date))  %>% distinct() %>% group_by( Week) %>% summarise(TS1_tower_wl = mean(wl, na.rm=T)) 
...
se.wl.df <- read.csv('data/data_raw/SE_WaterLevel_2020_2022.csv') # se.wl.df
se.wl.df <- se.wl.df %>% mutate( wl =wl.corr)

load("data/data_raw/se1_Data_AllData.RDATA") # Met data

se1.salinity <- se1 %>% 
  mutate(TIMESTAMP = as.POSIXct(TIMESTAMP, format= "%Y-%m-%d %H:%M:%S", tz="EST")) %>% 
  subset(date >= "2021-01-01" & date < "2022-01-01") %>% 
...
se1.wl <- se.wl.df %>% mutate(TIMESTAMP = as.POSIXct(TIMESTAMP,format= "%Y-%m-%d %H:%M:%S", tz="EST"),
  ...
  subset (date >= "2021-01-01" & date < "2022-01-01") %>% as.data.frame() %>% distinct() %>% mutate(wl=wl.corr)
